...
public class SnowyDirtBakedModel implements BakedModel {
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction direction, RandomSource random, ModelData extraData, @Nullable net.minecraft.client.renderer.RenderType renderType) {
        Boolean isSnowBlockBiome = extraData.get(IS_SNOW_BLOCK_BIOME_PROPERTY);
        Integer rotation = extraData.get(ROTATION_PROPERTY);

        if (isSnowBlockBiome != null && isSnowBlockBiome) {
            if (rotation != null && direction == Direction.UP) {
                RandomSource rotationRandom = RandomSource.create(rotation);
                return snowyModel.getQuads(state, direction, rotationRandom, extraData, renderType);
            }
    ...
    public ModelData getModelData(BlockAndTintGetter level, BlockPos pos, BlockState state, ModelData modelData) {
        ...
        try {
            ...
            if (mc.level != null && pos != null) {
                isSnowBlockBiome = BiomeTemperatureHelper.isSnowBlockBiome(mc.level, pos);

                rotation = (pos.getX() * 3 + pos.getY() * 5 + pos.getZ() * 7) & 0xFFFF;
            }
        } catch (Exception e) {
        }
        ...
        return modelData.derive()
                ...
                .with(IS_SNOW_BLOCK_BIOME_PROPERTY, isSnowBlockBiome)
                .with(ROTATION_PROPERTY, rotation)
                .build();
    }
}